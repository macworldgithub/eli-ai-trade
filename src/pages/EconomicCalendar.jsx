...
const StarsBadge = ({ stars }) => {
  ...
  return (
    <span
      className={`px-2.5 sm:px-3 py-0.5 text-[10px] sm:text-xs font-bold border rounded-sm tracking-wider ${color}`}
    >
      {Array(stars).fill("★").join("")}
    </span>
  );
...
const ImpactBadge = ({ impact }) => {
  ...
  return (
    <span
      className={`px-2.5 sm:px-3 py-0.5 text-[10px] sm:text-xs font-bold border rounded-sm ${colors[impact] || colors.LOW}`}
    >
      {impact} IMPACT
    </span>
  );
...
export default function EconomicCalendar() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const mockIndicesEvents = [
        {
          ...
          symbol: "SPX",
          description:
            "Quarterly futures contract rollover affecting S&P 500 liquidity and volume.",
          aiVolatility: 1.2,
        ...
        },
        {
          ...
          symbol: "NDX",
          description:
            "Key earnings reports from major NASDAQ 100 constituents expected after market close.",
          aiVolatility: 2.5,
        ...
        },
        {
          ...
          symbol: "UKX",
          description:
            "Index weighting adjustments for UK large caps taking effect at open.",
          aiVolatility: 0.8,
        ...
        },
        {
          ...
          symbol: "ASX",
          description:
            "Quarterly review of banking sector capital requirements.",
          aiVolatility: 0.6,
      ...
      const allEvents = [...apiEvents, ...mockIndicesEvents]
        .map((e) => ({
          ...e,
          historic: e.historic || {
  ...
  return (
    <div className="space-y-6 px-3 sm:px-4 lg:px-6" data-testid="calendar-page">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <Calendar className="w-7 h-7 sm:w-8 sm:h-8 text-eli-gold" />
            <h1 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-bold text-eli-text-white">
              Economic Calendar
            </h1>
          </div>
          <p className="text-eli-muted mt-1 text-sm sm:text-base">
        ...
        <button
          onClick={load}
          disabled={loading}
          className="flex items-center gap-2 px-5 py-2.5 bg-eli-gold hover:bg-eli-gold-bright text-eli-navy font-bold rounded-sm disabled:opacity-50 text-sm sm:text-base whitespace-nowrap"
        >
          <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
          Refresh
        </button>
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3">
        {["ALL", "HIGH", "MEDIUM"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 sm:px-5 py-2 text-xs sm:text-sm font-bold uppercase tracking-wider rounded-sm border transition-all whitespace-nowrap ${
              filter === f
                ? "bg-eli-gold text-eli-navy border-eli-gold"
        ...
        <div className="w-px bg-eli-border mx-2 self-center hidden sm:block" />
      ...
      {loading ? (
        ...
        <div className="eli-card p-12 sm:p-16 text-center">
          <Calendar className="w-14 h-14 sm:w-16 sm:h-16 text-eli-gold/40 mx-auto mb-4" />
          <h3 className="text-xl sm:text-2xl font-bold text-eli-text-white">
            No matching events
          </h3>
        </div>
      ) : (
        <div className="space-y-8">
          {Object.entries(groups).map(([day, evs]) => (
            <div key={day}>
              <h2 className="text-base sm:text-lg lg:text-xl font-bold text-eli-gold tracking-wider mb-4 px-1">
                {day}
              </h2>
              <div className="eli-card divide-y divide-eli-border">
                {evs.map((e) => (
                  <div
                    ...
                    className="px-4 sm:px-6 py-4 sm:py-5 hover:bg-eli-border/50 transition-all cursor-pointer group"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      <div className="font-mono text-xs sm:text-sm text-eli-muted w-20 shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <button
                            ...
                            className="font-bold text-eli-text-white text-sm sm:text-[15px] hover:text-eli-gold transition-colors text-left flex items-center gap-1.5"
                          >
                            {e.title}
                            <ExternalLink className="w-3.5 h-3.5 opacity-50" />
                          </button>
                          {e.currency && (
                            <span className="text-eli-gold font-mono text-xs sm:text-sm">
                              ({e.currency})
                            </span>
                          )}
                        </div>
                        {e.description && (
                          <p className="text-xs text-eli-muted mt-1 line-clamp-2 sm:line-clamp-1">
                            {e.description}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                        ...
                        {/* AI Volatility - ONLY when INDICES filter is active */}
      ...
      {/* Event Detail Modal */}
      {selectedEvent && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-3 sm:p-6">
          <div className="eli-card max-w-2xl w-full max-h-[92vh] overflow-auto">
            <div className="p-5 sm:p-6 border-b border-eli-border flex items-center justify-between sticky top-0 bg-eli-navy z-10">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-eli-text-white">
                  {selectedEvent.title}
                </h2>
                <p className="text-eli-muted mt-1 text-sm">
            ...
            <div className="p-5 sm:p-6 space-y-6 sm:space-y-7">
              <div className="flex flex-wrap gap-3">
                ...
                {selectedEvent.aiVolatility && (
                  <div className="flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-sm text-sm">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-eli-navy-3 border border-eli-border rounded-sm p-4 text-center">
                  <div className="text-[10px] uppercase tracking-wider text-eli-muted mb-1">
                    Previous
                  </div>
                  <div className="font-mono text-base sm:text-lg font-bold text-eli-text-white">
                ...
                <div className="bg-eli-navy-3 border border-eli-border rounded-sm p-4 text-center relative overflow-hidden">
                  ...
                  <div className="font-mono text-base sm:text-lg font-bold text-eli-gold">
                ...
                <div className="bg-eli-navy-3 border border-eli-border rounded-sm p-4 text-center">
                  ...
                  <div className="font-mono text-base sm:text-lg font-bold text-eli-text-white">
                    {selectedEvent.actual || "TBD"}
                  </div>
                </div>
              </div>

              {selectedEvent.description && (
                <p className="text-sm sm:text-[15px] text-eli-slate-300 leading-relaxed bg-eli-border/20 p-5 rounded-sm border border-eli-border/50">
                  {selectedEvent.description}
                </p>
              )}

              {selectedEvent.historic && (
                <div className="bg-eli-border/40 border border-eli-border rounded-sm p-5 sm:p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`font-mono text-sm font-bold px-3 py-1 rounded-sm ${selectedEvent.historic.direction === "up" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
                    >
                      {selectedEvent.historic.direction === "up" ? "+" : "-"}
                      {selectedEvent.historic.move}%
                    </span>
                  </div>

                  <div className="h-44 sm:h-52 mt-4 bg-eli-navy-3/80 border border-eli-border rounded flex items-end justify-between px-3 pt-6 pb-2 relative group cursor-crosshair">
                    <div className="absolute top-2 left-3 text-[10px] text-eli-muted font-mono tracking-widest uppercase">
                      {selectedEvent.relatedSymbol ||
                        `${selectedEvent.currency || "USD"}`}{" "}
                      • 5m Post-Event Snapshot
                    </div>

                    {selectedEvent.historic.chart.map((candle, i) => (
                      <div
                        key={i}
                        className="flex flex-col items-center justify-end w-3 h-full relative"
                      >
                        <div
                          className={`w-[1px] absolute top-2 bottom-2 ${candle.isUp ? "bg-emerald-500/30" : "bg-red-500/30"}`}
                        />
                        <div
                          ...
                          style={{
                            ...
                            marginBottom: `${candle.bottomOffset}%`,
                          }}
                        />
                      </div>
                    ))}
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-eli-border">
                <button
                  onClick={() => openReferencedMarket(selectedEvent)}
                  className="flex-1 py-3.5 rounded-sm font-bold border border-eli-overlay-20 hover:bg-eli-overlay-10 flex items-center justify-center gap-2 text-sm sm:text-base"
                >
                  <BarChart3 className="w-4 h-4" />
                  Open in Markets
                </button>
                <button
                  ...
                  className="flex-1 bg-eli-gold hover:bg-eli-gold-bright text-eli-navy py-3.5 rounded-sm font-bold flex items-center justify-center gap-2 text-sm sm:text-base"
                >
                  <Target className="w-4 h-4" />
                  Risk Calculator
                </button>
