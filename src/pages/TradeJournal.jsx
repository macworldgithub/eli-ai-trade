...
// ============== Stat card ==============
const Stat = ({ label, value, color = "text-eli-text-white" }) => (
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-[10px] mb-1">
        <span className="text-eli-muted uppercase tracking-wider">{label}</span>
        <span className="text-eli-gold font-mono font-bold">{score}/10</span>
      </div>
      <div className="h-1 bg-eli-border rounded-full overflow-hidden">
        <div
...
const CoachBlock = ({ coach }) => {
  ...
  return (
    <div className="mt-3 p-4 bg-eli-navy border border-eli-gold/30 rounded-sm">
      <div className="flex items-center gap-2 mb-4">
        <Sparkles className="w-5 h-5 text-eli-gold" />
        <h4 className="text-sm font-bold text-eli-gold uppercase tracking-wider">
          AI Coach Feedback
        </h4>
        <span className="ml-auto text-sm font-mono text-eli-text-white">
          Overall: <span className="text-eli-gold font-bold text-lg">{coach.overall_score}/10</span>
        </span>
      </div>
      ...
      <div className="space-y-3 text-sm">
        <div className="bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-sm">
          ...
          <span className="text-eli-slate-300">{coach.what_you_did_well}</span>
        </div>
        <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-sm">
          <span className="text-amber-400 font-bold block mb-1">⚠ Areas to improve: </span>
          <span className="text-eli-slate-300">{coach.areas_to_improve}</span>
        </div>
        <div className="bg-eli-gold/10 border border-eli-gold/20 p-3 rounded-sm">
          <span className="text-eli-gold font-bold block mb-1">★ Key lesson: </span>
          <span className="text-eli-slate-300">{coach.key_lesson}</span>
        </div>
      </div>
    </div>
  );
...
const LogTradeForm = ({ instruments, verdicts, prefill, onCreated, onClearPrefill }) => {
  ...
  return (
    <div className="eli-card border-eli-gold/40" data-testid="log-trade-form">
      <div className="px-6 py-4 border-b border-eli-border flex items-center justify-between bg-eli-navy">
        <div className="flex items-center gap-3">
          <BookOpen className="w-5 h-5 text-eli-gold" />
          <h2 className="font-bold text-eli-text-white tracking-wider">Log New Trade</h2>
        </div>
        {live != null && (
          <div className="flex items-center gap-2 text-sm text-eli-muted font-mono bg-eli-border/30 px-3 py-1 rounded-full border border-eli-border">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1.5fr] gap-4">
          <div>
            <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Instrument</label>
            <select
              value={f.instrument}
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Direction</label>
            <div className="grid grid-cols-2 gap-2">
              {DIRECTIONS.map((d) => (
                <button
                  ...
                  type="button"
                  onClick={() => handleDir(d)}
                  className={`py-3 text-sm font-bold rounded-sm border transition-colors ${f.direction === d
                    ? d === "LONG"
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Entry Price</label>
            <div className="flex gap-2">
              <input
                ...
                placeholder="0.00000"
                className="flex-1 px-4 py-3 bg-eli-border/40 border border-eli-border rounded-sm text-eli-text-white font-mono focus:outline-none focus:ring-2 focus:ring-eli-gold/50"
              />
              <button
                type="button"
                onClick={snap}
                ...
                className="px-4 bg-eli-gold/10 hover:bg-eli-gold/20 text-eli-gold rounded-sm border border-eli-gold/40 disabled:opacity-30"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="0.00000"
              className="w-full px-4 py-3 bg-eli-border/40 border border-red-500/30 rounded-sm text-eli-text-white font-mono focus:outline-none focus:ring-2 focus:ring-red-500/50"
            />
          </div>
          <div>
            ...
            <input
              ...
              placeholder="0.00000"
              className="w-full px-4 py-3 bg-eli-border/40 border border-emerald-500/30 rounded-sm text-eli-text-white font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
            />
          </div>
          <div className="flex flex-col">
            <label className="block text-xs uppercase tracking-wider text-eli-gold mb-1.5 flex items-center gap-1.5">
              <Target className="w-4 h-4" /> Risk : Reward
            </label>
            <div className="flex-1 bg-eli-navy border border-eli-gold/30 rounded-sm px-4 py-3 flex items-center justify-between">
              {rr ? (
                <>
                  <div className="text-sm">
                    ...
                    <span className="text-eli-muted mx-2">→</span>
                    <span className="text-emerald-400 font-mono">
                      {decimals === 5 ? `${(rr.reward * rr.pipMult).toFixed(1)}p` : rr.reward.toFixed(decimals)}
                    </span>
                  </div>
                  <span
                    className={`font-mono font-bold text-lg ${rr.ratio >= 2 ? "text-emerald-400" : rr.ratio >= 1 ? "text-amber-400" : "text-red-400"
                      }`}
                  >
                ...
                <span className="text-sm text-eli-muted">Fill entry + SL + TP</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Position Size</label>
            <input
              type="number"
              step="0.01"
              value={f.position_size}
              onChange={(ev) => setF({ ...f, position_size: ev.target.value })}
              className="w-full px-4 py-3 bg-eli-border/40 border border-eli-border rounded-sm text-eli-text-white font-mono focus:outline-none focus:ring-2 focus:ring-eli-gold/50"
            />
          </div>
          <div className="flex flex-col">
            <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Dollar Impact ($ Risk)</label>
            <div className="flex-1 bg-eli-border/30 border border-eli-border rounded-sm px-4 py-3 flex items-center justify-between text-eli-text-white font-mono">
              <div className="flex flex-col">
                <span className="text-xs text-eli-muted mb-1">~$10/pip per lot approx.</span>
                <span className="text-lg font-bold text-red-400">
                  ${dollarRisk > 0 ? dollarRisk.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0.00"}
                </span>
              </div>
              <div className="text-right flex flex-col">
                <span className="text-xs text-eli-muted mb-1">Pip Dist.</span>
                <span className="text-eli-text-white">{pipDistance.toFixed(1)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="text-sm text-eli-gold hover:text-eli-text-white flex items-center gap-2 font-bold"
        >
        ...
        {showRiskCalc && (
          <div className="border border-eli-border rounded-sm p-4 bg-eli-navy/50">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-eli-gold hover:text-eli-text-white font-bold pt-2 border-t border-eli-border/50 w-full"
        >
        ...
        {detailsOpen && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-eli-border">
            <div>
              <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Session</label>
              <select
                value={f.session}
                onChange={(ev) => setF({ ...f, session: ev.target.value })}
                className="w-full px-4 py-3 bg-eli-border/40 border border-eli-border rounded-sm text-eli-text-white"
              >
                {SESSIONS.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Entry Model</label>
              <select
                value={f.entry_model}
            ...
            <div className="md:col-span-2">
              <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5 flex items-center gap-1">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Entry Zone (POI / Timeframe)</label>
              <input
                type="text"
                value={f.entry_zone}
            ...
            <div className="md:col-span-2">
              <label className="block text-xs uppercase tracking-wider text-eli-muted mb-1.5">Notes</label>
              <textarea
                value={f.notes}
    ...
    >
      <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <span
            className={`px-2 py-1 text-xs font-bold border rounded-sm tracking-wider flex items-center ${trade.direction === "LONG"
          ...
          <span className="font-mono text-lg font-bold text-eli-text-white">{trade.instrument}</span>
          <span className="text-xs text-eli-muted px-2 py-1 bg-eli-border/50 rounded-sm">{trade.session}</span>
          <span className="text-xs text-eli-gold px-2 py-1 bg-eli-gold/10 rounded-sm">{trade.entry_model}</span>
        </div>

        <div className="flex items-center gap-3">
          <span
            className={`px-3 py-1 text-xs font-bold rounded-sm ${isOpen
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm bg-eli-navy p-4 rounded-sm border border-eli-border/50 mb-3">
        <div>
          <span className="text-eli-muted block text-[10px] uppercase mb-1">Entry Price</span>
          <span className="font-mono text-eli-text-white text-base">{trade.entry_price?.toFixed(dec)}</span>
        </div>
        <div>
          <span className="text-eli-muted block text-[10px] uppercase mb-1">Stop Loss</span>
          <span className="font-mono text-red-400 text-base">{trade.stop_loss?.toFixed(dec)}</span>
        </div>
        <div>
          <span className="text-eli-muted block text-[10px] uppercase mb-1">Take Profit</span>
          <span className="font-mono text-emerald-400 text-base">{trade.take_profit?.toFixed(dec)}</span>
        </div>
        <div>
          <span className="text-eli-muted block text-[10px] uppercase mb-1">Position Size</span>
          <span className="font-mono text-eli-text-white text-base">{trade.position_size}</span>
        </div>
      </div>
      ...
      {(trade.strategy_tags?.length > 0 || trade.chart_url) && (
        <div className="flex flex-wrap items-center gap-3 mt-3 mb-2">
          {trade.strategy_tags?.map(tag => (
            <span key={tag} className="text-[10px] px-2 py-0.5 bg-eli-border text-eli-slate-300 rounded-sm flex items-center gap-1">
              <Tag className="w-3 h-3 text-eli-muted" /> {tag}
            </span>
          ))}
          {trade.chart_url && (
            <a href={trade.chart_url} target="_blank" rel="noopener noreferrer" className="text-[10px] px-2 py-0.5 bg-eli-gold/10 text-eli-gold hover:bg-eli-gold/20 border border-eli-gold/30 rounded-sm flex items-center gap-1 transition-colors">
      ...
      {trade.entry_zone && (
        <p className="text-sm text-eli-muted mt-2">
          <span className="text-eli-gold uppercase tracking-wider mr-2 text-[10px] font-bold">POI:</span>
          {trade.entry_zone}
        </p>
      )}

      {trade.notes && <p className="text-sm text-eli-slate-300 mt-2 italic border-l-2 border-eli-border pl-3 py-1">"{trade.notes}"</p>}

      {isOpen && (
        <div className="mt-4 pt-4 border-t border-eli-border">
          {showClose ? (
            <div className="flex gap-3 items-end bg-eli-navy p-3 border border-eli-border rounded-sm">
              <div className="flex-1">
                <label className="block text-[10px] text-eli-muted uppercase tracking-wider mb-1.5">
                  Exit Price
                </label>
                <input
                  type="number"
                  ...
                  className="w-full px-3 py-2 bg-eli-border/40 border border-eli-border rounded-sm text-eli-text-white font-mono text-sm"
                />
              </div>
              <button
                onClick={handleClose}
                disabled={!exitPrice}
                className="px-5 py-2 bg-eli-gold text-eli-navy font-bold rounded-sm disabled:opacity-50 hover:bg-eli-gold-bright transition-colors"
              >
                Confirm Close
              </button>
              <button onClick={() => setShowClose(false)} className="px-4 py-2 text-sm text-eli-muted hover:text-eli-text-white">
                Cancel
              </button>
            </div>
          ) : (
            <button
              ...
              className="text-sm px-4 py-2 bg-eli-border/50 text-eli-text-white hover:bg-eli-border rounded-sm flex items-center gap-2 border border-eli-border"
            >
      ...
      {!isOpen && (
        <>
          <div className="mt-4 pt-4 border-t border-eli-border flex items-center justify-between gap-2 flex-wrap">
            <div className="text-sm bg-eli-border/30 px-3 py-1.5 rounded-sm border border-eli-border">
              <span className="text-eli-muted mr-2">Exit Price:</span>
              <span className="font-mono text-eli-text-white font-bold">{trade.exit_price?.toFixed(dec)}</span>
            </div>
            {!trade.ai_coach && (
              <button
                onClick={() => onScore(trade.id)}
                disabled={scoringId === trade.id}
                className="flex items-center gap-2 text-sm px-4 py-1.5 bg-eli-gold/10 text-eli-gold border border-eli-gold/30 hover:bg-eli-gold/20 rounded-sm transition-colors disabled:opacity-50"
              >
  ...
  return (
    <div className="space-y-8" data-testid="journal-page">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <BookOpen className="w-8 h-8 text-eli-gold" />
            <h1 className="font-heading text-4xl font-bold text-eli-text-white">Trade Journal</h1>
          </div>
        ...
        <button
          onClick={exportToCSV}
          className="flex items-center gap-2 px-4 py-2 bg-eli-border/50 hover:bg-eli-border text-eli-text-white border border-eli-border rounded-sm transition-colors"
        >
      ...
      <div className="eli-card p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <Sparkles className="w-6 h-6 text-eli-gold" />
            <h3 className="text-xl font-bold text-eli-text-white">AI Pattern Insight</h3>
          </div>
          <button
            onClick={runPatterns}
            disabled={loadingPatterns || closedTrades.length === 0}
            className="px-4 py-2 text-sm bg-eli-gold/10 text-eli-gold border border-eli-gold/30 hover:bg-eli-gold/20 rounded-sm flex items-center gap-2 disabled:opacity-50 transition-colors"
          >
        ...
        {!patterns ? (
          <div className="text-center p-6 bg-eli-navy border border-eli-border rounded-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-eli-navy p-5 border border-eli-border rounded-sm">
            <div className="md:col-span-3">
              <p className="text-eli-text-white text-lg">{patterns.patterns}</p>
            </div>
            {patterns.top_strength && (
              <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-sm">
                ...
                <span className="text-eli-text-white">{patterns.top_strength}</span>
              </div>
            )}
            {patterns.top_weakness && (
              <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-sm">
                ...
                <span className="text-eli-text-white">{patterns.top_weakness}</span>
              </div>
            )}
            {patterns.habits?.length > 0 && (
              <div className="bg-eli-border/30 border border-eli-border p-4 rounded-sm md:col-span-1">
                <span className="text-eli-muted font-bold block mb-2 uppercase text-xs tracking-wider">Observed Habits</span>
                <ul className="list-disc list-inside text-sm text-eli-slate-300 space-y-1">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <section data-testid="open-section">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-eli-gold tracking-wider uppercase flex items-center gap-2">
              <Activity className="w-5 h-5" /> Open Positions
              <span className="text-eli-muted font-mono bg-eli-border px-2 py-0.5 rounded-sm text-sm">{openTrades.length}</span>
            </h2>
          </div>
          {openTrades.length === 0 ? (
            <div className="eli-card p-10 text-center flex flex-col items-center justify-center">
              <Activity className="w-10 h-10 text-eli-border mb-3" />
              <p className="text-eli-muted">No open positions. Log one above.</p>
            </div>
          ) : (
        ...
        <section data-testid="closed-section">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-eli-gold tracking-wider uppercase flex items-center gap-2">
              ...
              <span className="text-eli-muted font-mono bg-eli-border px-2 py-0.5 rounded-sm text-sm">{closedTrades.length}</span>
            </h2>
          </div>
          {closedTrades.length === 0 ? (
            <div className="eli-card p-10 text-center flex flex-col items-center justify-center">
              <CheckCircle2 className="w-10 h-10 text-eli-border mb-3" />
              <p className="text-eli-muted">No closed trades yet.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {closedTrades.map((t) => (
